...
.login-card{background:#fff;border-radius:16px;padding:40px 36px;width:100%;max-width:400px;box-shadow:0 8px 40px rgba(0,0,0,.12);text-align:center}
.login-logo{font-size:28px;font-weight:800;color:#1B3A6B;letter-spacing:-.5px;margin-bottom:4px}
.login-sub{font-size:13px;color:#6B7280;margin-bottom:28px}
.login-input{width:100%;padding:11px 14px;border:1.5px solid #E5E7EB;border-radius:10px;font-size:14px;outline:none;margin-bottom:12px;font-family:inherit;transition:border-color .15s}
.login-input:focus{border-color:#2D6BE4}
.login-btn{width:100%;padding:12px;background:#1B3A6B;color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;transition:.15s;font-family:inherit}
.login-btn:hover{background:#2D6BE4}
.login-btn:disabled{opacity:.6;cursor:not-allowed}
.login-err{font-size:12px;padding:8px 12px;border-radius:8px;margin-bottom:12px;text-align:left;display:none}
.login-err.show{display:block}
.login-err.error{background:#FEE2E2;color:#991B1B}
.login-err.info{background:#EFF6FF;color:#1D4ED8}
.login-err.success{background:#DCFCE7;color:#15803D}
.login-toggle{font-size:12px;color:#6B7280;margin-top:14px;cursor:pointer}
.login-toggle span{color:#2D6BE4;font-weight:600;cursor:pointer}
...
<div id="login-root">
  <div class="login-card">
    ...
    <div class="login-err" id="login-msg"></div>
    <div id="login-nome-wrap" style="display:none">
      <input class="login-input" type="text" id="li-nome" placeholder="Nome completo">
    </div>
    ...
    <div class="login-toggle" id="login-toggle">
      Não tem conta? <span onclick="toggleLoginMode()">Criar conta</span>
    </div>
  </div>
</div>
...
function setLoginLoading(v){{
  ...
  if(btn){{btn.disabled=v;btn.textContent=v?'Aguarde...':loginMode==='login'?'Entrar':'Criar conta';}}
}}
function showLoginMsg(msg,type='error'){{
  const el=document.getElementById('login-msg');
  if(!el)return;
  el.textContent=msg;el.className='login-err show '+type;
}}
function clearLoginMsg(){{
  const el=document.getElementById('login-msg');
  if(el){{el.className='login-err';el.textContent='';}}
}}
function toggleLoginMode(){{
  loginMode=loginMode==='login'?'register':'login';
  const nomeWrap=document.getElementById('login-nome-wrap');
  const btn=document.getElementById('login-btn');
  const toggle=document.getElementById('login-toggle');
  if(nomeWrap)nomeWrap.style.display=loginMode==='register'?'block':'none';
  if(btn)btn.textContent=loginMode==='login'?'Entrar':'Criar conta';
  if(btn)btn.onclick=loginMode==='login'?doLogin:doRegister;
  if(toggle)toggle.innerHTML=loginMode==='login'?'Não tem conta? <span onclick="toggleLoginMode()">Criar conta</span>':'Já tem conta? <span onclick="toggleLoginMode()">Entrar</span>';
  clearLoginMsg();
}}
...
async function doLogin(){{
  ...
  setLoginLoading(true);clearLoginMsg();
  const {{data,error}}=await sb.auth.signInWithPassword({{email,password:pw}});
  setLoginLoading(false);
  if(error){{showLoginMsg(error.message==='Invalid login credentials'?'E-mail ou senha incorretos.':error.message);return;}}
...
async function doRegister(){{
  ...
  if(pw.length<6){{showLoginMsg('Senha mínimo 6 caracteres.');return;}}
  setLoginLoading(true);clearLoginMsg();
  const {{data,error}}=await sb.auth.signUp({{email,password:pw,options:{{data:{{nome}}}}}});
  setLoginLoading(false);
  if(error){{showLoginMsg(error.message);return;}}
  // Insert user in usuarios_app
  try{{await sb.from('usuarios_app').insert({{nome,email,cargo:'Usuário',ativo:true}});}}catch(e){{}}
  showLoginMsg('Conta criada! Verifique seu e-mail para confirmar.','success');
}}
